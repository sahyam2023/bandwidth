...
import {
  X, Server, Zap, Loader2, AlertTriangle, Info, BookOpen, ArrowRightLeft, Search, MemoryStick, Cpu, HardDrive
} from 'lucide-react';
...
function PeerTrafficPage() {
 ...
 const fetchData = useCallback(async () => {
  ...
  const handleNodeDragEnd = useCallback((node: GraphNode) => { node.fx = node.x; node.fy = node.y; setFixedNodeIds(prev => new Set(prev).add(node.id as string)); }, []);
  const handleNodeRightClick = useCallback((node: GraphNode) => { node.vx = 0; node.vy = 0; node.fx = undefined; node.fy = undefined; setFixedNodeIds(prev => { const newSet = new Set(prev); newSet.delete(node.id as string); return newSet; }); }, []);
  ...
  return (
    ...
    <div className="relative p-4 sm:p-6 flex flex-col h-full bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      <div className="mb-2 px-1 text-xs text-gray-600 dark:text-gray-400 flex flex-wrap gap-x-4 gap-y-1 border-b dark:border-gray-700 pb-1">
          ...
          <span>Total Peer Traffic: <strong className="text-gray-800 dark:text-gray-200">{summaryStats.aggregatePeerMbps.toFixed(1)} Mbps</strong></span>
      </div>
      ...
      <div ref={containerRef} className={` flex-grow border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow relative overflow-hidden ${ graphAreaReady && !loading && !error && fullGraphData.nodes.length > 0 ? 'opacity-100' : 'opacity-0' } `} style={{ minHeight: '300px', transition: 'opacity 0.3s ease-in-out' }} >
        {graphAreaReady && !loading && !error && fullGraphData.nodes.length > 0 && (
          <ForceGraph2D<GraphNode, GraphLink>
            ...
            // Links
            linkWidth={(link: GraphLink) => Math.min(MAX_LINK_WIDTH, MIN_LINK_WIDTH + (link.rate_mbps || 0) * LINK_WIDTH_SCALE)}
            // --- UPDATED: Link Color for highlighting ---
            linkColor={(link: GraphLink) => {
                ...
                const baseColor = link.type === 'peer_traffic' ? LINK_COLOR_PEER : LINK_COLOR_REPORTING;
                if (!fade) return baseColor;
                ...
                try {
                    ...
                    return `rgba(128, 128, 128, ${LINK_FADE_OPACITY})`; // Fallback faded color
                }
            }}
            linkLabel={(link: GraphLink) => {
              ...
              const rateString = link.type === 'peer_traffic'
                ? `Rate: <strong>${link.rate_mbps?.toFixed(2) ?? 'N/A'} Mbps</strong>`
                : 'Reporting Link';
              return `
            ...
            // --- UPDATED: Arrow Color for highlighting ---
            ...
            // Particles
            linkDirectionalParticles={(link: GraphLink) => (link.rate_mbps || 0) >= PARTICLE_MIN_RATE_MBPS ? PARTICLE_COUNT : 0}
            linkDirectionalParticleWidth={PARTICLE_WIDTH}
            // --- UPDATED: Particle Color for highlighting ---
            ...
            linkDirectionalParticleSpeed={(link: GraphLink) => Math.max(PARTICLE_SPEED_MIN, Math.min(PARTICLE_SPEED_MAX, (link.rate_mbps || 0) * PARTICLE_SPEED_SCALE))}
          />
        )}
      ...
      >
                  ...
                  {!selectedNodeData.is_collector && (
                    <div className="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2"> {/* Reduced padding/margin */}
                      <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-1.5 flex items-center gap-1.5">
                        <ArrowRightLeft size={16} /> Peer Connections:
                      </h3>
                      {/* Aggregate Stats */}
                      ...
                      {selectedPeerTraffic.peers.length > 0 ? (
                        ...
                        <p className="text-gray-500 dark:text-gray-400 italic text-xs">No active peer traffic.</p>
                      )}
                    </div>
                  )}
        ...
        <div className="absolute bottom-4 left-4 bg-gray-100 dark:bg-gray-800 bg-opacity-90 dark:bg-opacity-80 border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 text-xs p-2 rounded-md shadow-lg pointer-events-none z-30 max-w-[180px]">
            ...
            <div className="space-y-1">
                ...
                <div className="flex items-center">
                    <div style={{backgroundColor: LINK_COLOR_PEER}} className="w-4 h-1 mr-1.5 flex-shrink-0"></div>
                    <span>Peer Traffic Link</span>
                </div>
                <div className="flex items-center">
